...
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=64,
											 shuffle=True)
			  for x in ['valid1']}
dataset_sizes = {x: len(image_datasets[x]) for x in ['valid1']}
class_names = image_datasets['valid1'].classes
...
def train_model(model, criterion, optimizer, scheduler, num_epochs):
	...
	for epoch in range(num_epochs):
		...
		for phase in ['valid1']:
			...
			# deep copy the model
			if phase == 'valid1' and epoch_acc > best_acc:
				best_acc = epoch_acc
				best_model_wts = model.state_dict()
